...
$im_banners = array();
for ($i=1; $i<=3; $i++) {
    if (strlen($TO->get_option('banner'.$i))) $im_banners[] = $TO->get_option('banner'.$i); 
}
...
function scripts_method() {
	...
	//wp_register_script( 'googlejquery', TDU .'/js/jquery-1.9.1.min.js');
...
function display_waterfall_of_posts ( $cid = '', $except_pid = '', $page = '', $s = '', $tag = '' ) {
	global $wpdb, $matrix_cat_str, $im_banners;
    ...
    $w_found_posts = $w_posts->found_posts;
    $im_banner_count = count($im_banners);
    if ($im_banner_count > 0) {        
        $im_banner_order = 'byName';
        if ($w_found_posts <= $ppp)
            $im_banner_order = 'rand';
    }
	...
	if ( $w_posts->have_posts() ) {
        $w_post_counter = 1;
        if ($w_post_counter == 1 && !empty($page))
            $w_post_counter = ($page - 1) * $ppp + $w_post_counter;
...
function wp_get_cat_postcount($cid, $s="", $tag="") {
    global $matrix_cat, $wpdb;
    $args = array(
		'post_type' => 'post',
		'post_status' => 'publish'
	);
	...
	if ( isset($cid) ) {
	   $cats = get_categories('child_of='.$cid);       
       if ($cats) {           
           foreach($cats as $cat) {
            if (in_array($cat->term_id, $matrix_cat)) {
                $cid .= ','.$cat->term_id;
...
function custom_form_class_attr( $class ) {
    if ( is_page_template( 'page-students.php' ) )
        $class .= ' email-form cf';
    
	return $class;
}

add_action( 'wpcf7_before_send_mail', 'send_coupon' );
function send_coupon( $cf7 ) {    
    if ( $cf7->id ) {
        $email = $cf7->posted_data["student-email"];
        $student_page = get_page_by_path('students');
        if ( $student_page ) {
            $coupon = get_field('coupon', $student_page->ID);
            $subject = "imcreator.com coupon";
            $message = "Hi!\r\n\r\n";
            $message .= "Thank you for using IM Creator.\r\n";
            $message .= "Here is your student coupon:\r\n";
            $message .= $coupon ."\r\n\r\n";
            $message .= "Just enter this coupon when you checkout and you are all set.\r\n\r\n";
            $message .= "Love,\r\n";
            $message .= "The IM Creator Team.";
            $headers = 'From: Noreply <noreply@imcreator.com>' . "\r\n";            
            if ( $coupon ) {
